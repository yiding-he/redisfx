...
public class KeyTabController extends AbstractTabController {
  ...
  public TableColumn<KeyItem, Long> lengthColumn;
  ...
  private void listKeys0() {
    if (Str.isBlank(this.txtKeyPattern.getText())) {
      this.txtKeyPattern.setText("*");
    }
    ...
    try (Jedis jedis = JedisManager.getJedis()) {
      ...
      do {
        ...
        for (String key : result.getResult()) {
          ...
          long length = getLength(key, type, jedis);
  ...
  private long getLength(String key, String type, Jedis jedis) {
    switch (type) {
      case "string":
  ...
  public static class KeyItem {
    ...
    public KeyItem(String key, String type, long length, String expireAt) {
      setKey(key);
      setType(type);
      setLength(length);
